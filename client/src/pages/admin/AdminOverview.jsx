...
const PartnerOverviewTab = ({ partnerPerformances }) => {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 text-left font-semibold">Track jobs, average ratings, and revenue generated by each service partner</p>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
              {filteredPerformances.map(p => (
                <tr key={p.id || p.partnerName} className="hover:bg-slate-50/50">
                  ...
                  <td className="p-4 font-mono font-bold">{p.totalCompletedJobs || 0}</td>
                  <td className="p-4 font-mono text-emerald-600 font-bold">{p.completedJobs || p.totalCompletedJobs || 0}</td>
                  <td className="p-4 font-mono text-amber-600 font-bold">{p.activeJobs || 0}</td>
                  <td className="p-4 font-mono text-rose-600 font-bold">{p.cancelledJobs || 0}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-right font-mono font-bold text-brand">Rs. {(p.totalRevenue || 0).toLocaleString()}</td>
                </tr>
              ))}
...
export default function AdminOverview({ defaultTab = 'dashboard' }) {
  ...
  const loadSandboxTab = (tab) => {
    ...
    if (tab === 'dashboard') {
      ...
      setReviews([
        { id: 'rev-1', rating: 5, comment: 'Excellent service!', appreciation: 'Punctual and polite', complaint: '', createdAt: new Date().toISOString(), user: { name: 'Amit Patel' }, booking: { id: 'bk-12345678' }, partner: { name: 'Vijay Kumar' }, serviceCategory: 'Cleaning' },
        { id: 'rev-2', rating: 3, comment: 'Average cleaning, could be better.', appreciation: '', complaint: 'Left dust on corners', createdAt: new Date().toISOString(), user: { name: 'Sneha Reddy' }, booking: { id: 'bk-87654321' }, partner: { name: 'Ramesh Kumar' }, serviceCategory: 'Cleaning' }
      ]);
              ...
              >
                ...
                {activeTab === 'customer-ratings' && (
                  <div className="space-y-6 animate-fade-up">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1 text-left font-semibold">View verified reviews, appreciations, and complaints from customers</p>
                    </div>
                          ...
                          <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                            {reviews.map(r => (
                              <tr key={r.id} className="hover:bg-slate-50/50">
                                ...
                                <td className="p-4 text-slate-600 max-w-[200px] truncate" title={r.appreciation}>{r.appreciation || <span className="text-slate-300">-</span>}</td>
                                <td className="p-4 text-slate-600 max-w-[200px] truncate" title={r.complaint}>
                                  {r.complaint ? (
                                    <span className="text-rose-600 font-semibold bg-rose-50 px-2 py-0.5 rounded border border-rose-100">{r.complaint}</span>
                                  ) : (
                            ...
                            {reviews.length === 0 && (
                              <tr>
                                <td colSpan="8" className="p-8 text-center text-slate-400 font-medium">No reviews submitted yet</td>
                              </tr>
                            )}
                          </tbody>
                        </table>
                ...
                {activeTab === 'partner-overview' && (
                  <div className="animate-fade-up">
                    <PartnerOverviewTab partnerPerformances={partnerPerformances} />
                  </div>
                )}
